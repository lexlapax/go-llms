...
var dateTimeCalculateParamSchema = &schemaDomain.Schema{
	Type: "object",
...
func init() {
	// Register the tool
	if err := registerTool("datetime_calculate", DateTimeCalculate()); err != nil {
		panic(fmt.Sprintf("Failed to register datetime_calculate tool: %v", err))
	}
}
